...
def calc_probe_z_average(positions, method='average', axis=AXIS_Z_INDEX):
    if method != 'median':
        ...
        avg = [sum([pos[i] for pos in positions]) / count
               for i in range(3)]
    else:
        # Use median
        ...
        if (len(positions) & 1) == 1:
            ...
            avg = list(z_sorted[middle])[:3]
        else:
            # even number of samples
            avg = list(calc_probe_z_average(
                z_sorted[middle-1:middle+1], 'average', axis))[:3]
    return manual_probe.ProbeResult(avg[0], avg[1], avg[2],
                                    avg[0], avg[1], avg[2])
...
class ProbeEndstopWrapper:
    ...
    def probing_move(self, pos, speed):
        ...
        return phoming.probing_move(self, pos, speed, check_movement=False)
...
class ExtruderOffsetCalibration:
    ...
    def _motor_off(self, print_time=None):
        with self.lock:
